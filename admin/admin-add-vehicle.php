...
if(isset($_POST['add_veh'])) {
    ...
    $query = "INSERT INTO tms_vehicle (v_name, v_reg_no, v_driver, v_category, v_dpic, v_status) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
    if ($stmt) {
        $stmt->bind_param('ssssss', $v_name, $v_reg_no, $v_driver, $v_category, $v_dpic, $v_status);
        if ($stmt->execute()) {
            $succ = "Vehicle Added";
        } else {
            $err = "Failed to add vehicle: " . $stmt->error;
        }
    } else {
      ...
      <?php if(isset($succ)) {?>
        ...
        <div class="card-body">
          ...
          <form method ="POST" enctype="multipart/form-data"> 
            ...
            <div class="form-group">
                <label for="exampleInputEmail1">Facility Capacity</label>
                <input type="text" class="form-control" id="exampleInputEmail1" name="v_reg_no">
            </div>
            <div class="form-group">
                <label for="exampleInputEmail1">Facility Details</label>
                <input type="text" class="form-control" id="exampleInputEmail1" name="v_status">
            </div>
            <div class="form-group">
              <label for="exampleFormControlSelect1">Custodian</label>
              <select class="form-control" name="v_driver" id="exampleFormControlSelect1">
                <?php

                $ret="SELECT * FROM tms_user where u_category = 'Custodian' "; 
            ...
            <div class="form-group">
              <label for="exampleFormControlSelect1">Facility Category</label>
              <select class="form-control" name="v_category" id="exampleFormControlSelect1">
                <option>Academic Facilities</option>
                <option>Administrative Facilities</option>
                <option>Recreational Facilities</option>
                <option>Support Facilities</option>
                <option>Technology Facilities</option>
                <option>Specialized Facilities</option>
              </select>
            </div>
            ...
            <div class="form-group col-md-12">
                <label for="exampleInputEmail1">Facility Picture</label>
                <input type="file" class="btn btn-success" id="exampleInputEmail1" name="v_dpic">
            </div>

            <button type="submit" name="add_veh" class="btn btn-success">Add Facility</button>
          </form>
          <!-- End Form-->
        </div>
      </div>
